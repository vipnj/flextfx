...
package gs {
	...
	public class TweenLite {
		public static var version:Number = 6.0;
		...
		public function initTweenVals($hrp:Boolean = false, $reservedProps:String = ""):void {
			...
			if (this.target is Array) {
				...
				for (p in this.vars) {
					if (p == "ease" || p == "delay" || p == "overwrite" || p == "onComplete" || p == "onCompleteParams" || p == "onCompleteScope" || p == "runBackwards" || p == "onUpdate" || p == "onUpdateParams" || p == "onUpdateScope" || p == "autoAlpha" || p == "onStart" || p == "onStartParams" || p == "onStartScope" || p == "easeParams" || p == "mcColor" || p == "type" || ($hrp && $reservedProps.indexOf(" " + p + " ") != -1)) { //"type" is for TweenFilterLite, and it's an issue when trying to tween filters on TextFields which do actually have a "type" property.
		...
		public static function executeAll($e:Event):void {
			...
			for (p in _all) {
				for (tw in _all[p]) {
					if (_all[p][tw] != undefined && _all[p][tw].active) {
						_all[p][tw].render(t);
						if (_all[p] == undefined) { //Could happen if, for example, an onUpdate triggered a killTweensOf() for the object that's currently looping here. Without this code, we run the risk of hitting 1010 errors
							break;
						}
					}
				}
